...
function bcHandler (href, source) {
...
async function generateItems (breadcrumb) {
  ...
  for (const b of breadcrumb) {
    ...
    const content = b.content ? (' t:content="' + b.content + '"') : ''
...
const breadcrumb = {
  ...
  handler: async function (params = {}) {
    ...
    const { urlToBreadcrumb } = this.plugin.app.waibuMpa
    ...
    if (params.attr.autoFill && this.locals.breadcrumb) {
      ...
      const breadcrumb = urlToBreadcrumb(routePath(url), { handler: bcHandler, handlerScope: this })
      if (!params.attr.noHome) breadcrumb.unshift({ icon: 'house', href: '/' })
      params.html = await generateItems.call(this, breadcrumb)
    }
    params.attr = omit(params.attr, ['divider', 'autoFill', 'autoDetect', 'autoDetectHome'])
  }
}
