async function navSelectorDarkmode (params = {}) {
  ...
  const html = [
    await this.buildTag({ tag: 'dropdownItem', attr: { href: '?dark-mode=false', active: !this.req.darkMode }, html: this.req.t('Bright Mode') }),
    await this.buildTag({ tag: 'dropdownItem', attr: { href: '?dark-mode=true', active: this.req.darkMode }, html: this.req.t('Dark Mode') })
  ].join('\n')
  params.noTag = true
